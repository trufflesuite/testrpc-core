...
StateManager.prototype.personal_sign = function(address, dataToSign) {
  var account = this.accounts[to.hex(address).toLowerCase()];
  ...
  return sigUtil.personalSign(account.secretKey, { data: dataToSign });
};

StateManager.prototype.personal_ecRecover = function(dataThatWasSigned, signature) {
  return sigUtil.recoverPersonalSignature({ data: dataThatWasSigned, sig: signature });
};
