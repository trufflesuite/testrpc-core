    ...
    if (timestamp) {
      self.data.blocks.last(function(err, last) {
        ...
        if (last && to.number(last.header.timestamp) > timestamp) {
          self.logger.log(
            "Warning: Setting the block timestamp (" + timestamp + ") that is earlier than the parent block one."
          );
        }
      });
...
BlockchainDouble.prototype.setProtocolOptions = function(options) {
  if (typeof options.gasLimit !== "undefined") {
    this.blockGasLimit = options.gasLimit;
  }
